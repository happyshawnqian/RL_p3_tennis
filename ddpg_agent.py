...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents=1):
        ...
        # Make sure local and target networks share the same values at first
        for target, local in zip(self.actor_target.parameters(), self.actor_local.parameters()):
            target.data.copy_(local.data)

        # Critic Network (w/ Target Network)
        # Note that in MADDPG, critics see all agents' states and actions
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        the state may be multiple states
        note: this function is NOT used in MDDPG
        '''
        # save experience
    ...
    def learn(self, experiences, gamma):
        # for DDPG only
        # MADDPG uses maddpg.maddpg_learn()
        '''
